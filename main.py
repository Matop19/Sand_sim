...
#TODO : dans PARTICLESMANAGER, initialiser particles{} avec toutes les valeurs à None.
def window():
	...
	particlesManager.clearParticles((width, height))
	...
	while running:
		...
		particlesManager.updateAllParticles((width, height), blockSize)
		if toggleGrid:
			toggleGrid = False
			hasGrid = not hasGrid
		if(hasGrid):
					...
					for dx in range(-brush_size + 1, brush_size):
						for dy in range(-brush_size + 1, brush_size):
							...
							if 0 <= x < width - 150 and 0 <= y < height:
								if Sand.is_used:
									particlesManager.put_particle((x,y), Sand)
								if Water.is_used:
									particlesManager.put_particle((x,y), Water)
								if Lava.is_used:
									particlesManager.put_particle((x,y), Lava)
								if Steel.is_used:
									particlesManager.put_particle((x,y), Steel)
								if Rock.is_used:
									particlesManager.put_particle((x,y), Rock)
								if Glass.is_used:
									particlesManager.put_particle((x,y), Glass)
								else:
									pass
